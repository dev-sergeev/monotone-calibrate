...
def test_search_iteratively_evaluates_hypotheses_and_returns_a_frozen_portfolio() -> None:
    ...
    assert result.hypotheses_accepted == 2
    assert result.hypotheses_buffered == 2
...
def test_partial_sampling_failure_atomically_discards_the_random_portfolio() -> None:
    class PartiallyFailingSampler:
        ...
        def sample(self, _request):
            self.calls += 1
            if self.calls == 1:
                return (EquationHypothesis("P1", ("poly2_v1",)),)
            raise TimeoutError("provider detail must not escape")
    ...
    assert result.warning_codes == (
        "LLM_TRAINING_SUMMARY_DISCLOSED",
        "LLM_SR_PARTIAL_FAILURE",
    )


def test_search_keeps_the_complete_p1_baseline_when_llm_omits_logistic() -> None:
    class PolynomialOnlySampler:
        def sample(self, _request):
            return (
                EquationHypothesis("P1", ("poly2_v1",)),
                EquationHypothesis("P2", ("poly1_v1", "poly2_v1")),
    ...
    x = np.linspace(0.0, 80.0, 120)
    y = 1.5 + 18.0 / (1.0 + np.exp(-0.12 * (x - 38.0)))
    result = run_symbolic_search(
        x,
        y,
        _enabled_config(),
        FitOptions(),
        options=SymbolicSearchOptions(iterations=1),
        sampler=PolynomialOnlySampler(),
    )

    assert set(result.hypothesis_space.p1_family_ids) == set(FAMILY_IDS)
    fitted = fit_candidates(
        x,
        y,
        FitOptions(hypothesis_space=result.hypothesis_space),
    )
    assert fitted.one.model.family_ids == ("logistic_v1",)
