...
Для внутреннего выбора `candidate-search-v2` используется нижняя граница practical tie 1.5% и конечновыборочная поправка за лишние параметры. Для более простого кандидата допустимая относительная разница SSE равна `max(0.015, exp(Δk·log(n)/n)−1)`, то есть границе эквивалентности BIC; после этого выбирается complexity vector из модельного контракта. Такой guard не позволяет нескольким лишним степеням полинома объяснять шум малой выборки. Историческая production-policy v1 использовала фиксированный 1%.
